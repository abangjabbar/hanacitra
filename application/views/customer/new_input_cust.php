				...
				<div class="card mb-3">
					...
					<div class="card-body">
						...
						<form action="<?php echo site_url('orders/addNewOrderCust ') ?>" method="post" enctype="multipart/form-data" >
							...
							<div class="form-group">
								<label for="jumlah">Jumlah* <?php echo form_error('jumlah') ? 'is-invalid':'' ?> </label>
                           ...
                           <div class="form-group">
                                <label for="deskripsi">Quality*</label>
                                <select class="form-control" name="kualitas" id="kualitas" required>
                                    <option value="">No Selected</option>
                                    <?php foreach($kualitas->result() as $row):?>
                                    <option value="<?php echo $row->id_kualitas;?>"><?php echo $row->kualitas_name;?></option>
                                    <?php endforeach;?>
                                </select>
                            </div>

                            <div class="form-group">
                                <label for="deskripsi">Subkualitas*</label>
                                <select class="form-control" name="subkualitas" id="subkualitas" required>
                                    <option value="0">Select Subkualitas</option>
                                </select>
                            </div>

                            <div class="form-group">
                                <label for="deskripsi">Harga*</label>
                                <select class="form-control" name="harga_subkualitas" id="harga_subkualitas" required>
                                    <option value="0">Select Subkualitas</option>
                                </select>
                            </div>

                            <div class="form-group">
                                <label for="deskripsi">Kualitas*</label>
                                <select class="form-control" name="substances" id="substances" required>
                                    <option value="0">No Selected</option>
                                    <?php foreach($substances->result() as $row):?>
    ...
    <script>
        $(document).ready(function(){
            $('#kualitas').change(function() {
                var id_kualitas = $(this).val();
                $.ajax({
                    url: "<?= base_url(); ?>index.php/orders/get_subkualitas",
                    method: "POST",
                    dataType: "JSON",
                    data: {
                    ...
                    success: function(array){
                        ...
                        for(let index = 0; index < array.length; index++) {
                            html += "<option>" + array[index].subkualitas_nama + "</option>"

                        }
                        $('#subkualitas').html(html);
                    }
                })
        $('#subkualitas').change(function() {
                var id_subkualitas = $(this).val();
                $.ajax({
                    url: "<?= base_url(); ?>index.php/orders/get_harga_subkualitas",
                    method: "POST",
                    dataType: "JSON",
                    data: {
                    ...
                    success: function(array){
                        var html = '';
                        for(let index = 0; index < array.length; index++) {
                            html += "<option>" + array[index].harga + "</option>"

                        }
                        $('#harga_subkualitas').html(html);
                    }
                })
            })
            })
